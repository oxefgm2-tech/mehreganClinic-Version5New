...
export const PetShopMdiTab: React.FC<PetShopMdiTabProps> = ({
  ...
  const handleScanBarcode = () => {
    ...
    const foundProduct = products.find(
      (p) => String(p.barcode ?? '') === barcodeQuery.trim() || String(p.title ?? '').toLowerCase().includes(barcodeQuery.toLowerCase())
    );
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {products
                ...
                .filter((p) => String(p.title ?? '').toLowerCase().includes(productSearchQuery.toLowerCase()))
                .map((prod) => (
                  <div
                    key={prod.id}
            ...
            <div className="bg-white p-5 rounded-2xl border border-[#E6E9DF] shadow-xs space-y-4">
              ...
              <div className="space-y-2 pb-3 border-b border-[#E6E9DF]">
                ...
                {activeTab.type === 'customer_sale' && (
                  <div>
                    <label className="block text-[11px] font-bold text-[#738A6E] mb-1">سرپرست فاکتور / مشتری</label>
                    <select
