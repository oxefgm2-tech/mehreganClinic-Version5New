...
async function runSmokeTests() {
  ...
  await runTest('Migration Storage', 'GET migration-ready vaccination/inventory/accounting collections', async () => {
    const endpoints = [
      '/api/vaccinations',
      '/api/petshop/product-movements',
      '/api/finance/accounting-documents',
    ];
    for (const endpoint of endpoints) {
      const res = await request(endpoint);
      if (!res.ok || !res.data?.success || !Array.isArray(res.data?.data)) {
        throw new Error(`Invalid response from ${endpoint}: ${JSON.stringify(res.data)}`);
      }
    }
  });
  ...
  await runTest('Backup & Export', 'POST /api/backup/create creates physical file in data/backups/', async () => {
    ...
    const backupFile = path.join(path.dirname(STORE_PATH), 'backups', backupRes.data.backupFile);
